...
exports.image = function image(source, options) {
  ...
  html += cloudinary.utils.html_attrs(extend(localOptions, responsiveAttributes, attributes)) + "/>";
  return html;
};
...
exports.video = function video(public_id, options) {
  ...
  let video_options = cloneDeep(options);
  ...
  if (video_options.hasOwnProperty('poster')) {
    if (isPlainObject(video_options.poster)) {
      if (video_options.poster.hasOwnProperty('public_id')) {
        ...
        video_options.poster = cloudinary.utils.url(public_id, extend({}, cloudinary.utils.DEFAULT_POSTER_OPTIONS, video_options.poster));
      }
    }
  } else {
    video_options.poster = cloudinary.utils.url(public_id, extend({}, cloudinary.utils.DEFAULT_POSTER_OPTIONS, options));
  }
  ...
  if (multi_source) {
    html += source_types.map(source_type => {
      ...
      let src = cloudinary.utils.url(source + "." + source_type, extend({resource_type: 'video'}, cloneDeep(options), cloneDeep(transformation)));
