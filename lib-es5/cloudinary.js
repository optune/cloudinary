...
exports.video = function video(public_id, options) {
  ...
  var video_options = cloneDeep(options);
  ...
  if (multi_source) {
    html += source_types.map(function (source_type) {
      ...
      var src = cloudinary.utils.url(source + "." + source_type, extend({ resource_type: 'video' }, cloneDeep(options), cloneDeep(transformation)));
